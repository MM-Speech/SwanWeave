...
def apply_atempo_ffmpeg(wav: np.ndarray, sr: int = 24000, tempo: float = 1.1) -> np.ndarray:
    import subprocess
    ...
    with tempfile.NamedTemporaryFile(suffix='.wav', delete=False) as f:
        output_path = f.name
    try:
        sf.write(input_path, wav.astype(np.float32), sr)
        cmd = [
        ...
        result, _ = librosa.load(output_path, sr=sr)
        return result
    finally:
...
def _estimate_target_infer_length(
    ...
    if ref_audio is not None and isinstance(ref_audio, np.ndarray) and ref_audio.size > 0:
        ...
        # If we have ref_audio, we require a valid ref_text (ASR or provided) to
        # estimate speaking rate. Do not silently fall back to default CPM.
        if not (ref_units > 0 and ref_dur > 1e-3):
            raise RuntimeError(
                f"ref_audio is provided but ref_text is empty/invalid for rate estimation "
                f"(ref_units={ref_units}, ref_dur={ref_dur:.4f}s). "
                f"Provide ref_text or fix ASR; refusing to fall back to default_cpm={default_cpm}."
            )
        ...
        for t in range(num_iters):
            ...
            if (not is_dummy) and sample.get('prompt_audios'):
                ...
                for p in sample['prompt_audios']:
                    ...
                    a = apply_atempo_ffmpeg(a, sr=24000, tempo=1.1)
                    audios.append(a)
                ...
                audio = apply_atempo_ffmpeg(audio, sr=24000, tempo=1.1)
            else:
                audio = None
...
if __name__ == '__main__':
    ...
    with open(args.config, "r", encoding="utf-8") as f:
        cfg = yaml.safe_load(f)
    out_path = f'{cfg["out_path"]}/{os.path.basename(args.dit_ckpt)}_{cfg["bench"]}/{cfg.get("cfg_w", "noCFG")}_timew{cfg.get("timestep_annealing_w", (1.0, 0.0, 1.0))}_amo{cfg.get("use_amo_sampler", False)}_sway{cfg.get("use_sway", False)}_step{cfg.get("num_step", 100)}_cpm{cfg.get("cpm", 300.0)}_reftempo1.1_seedlength'
