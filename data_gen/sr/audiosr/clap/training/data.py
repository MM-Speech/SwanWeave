...
from data_gen.sr.audiosr.clap.open_clip.utils import get_tar_path_from_dataset_name
from data_gen.sr.audiosr.clap.open_clip.utils import load_class_label
...
from data_gen.sr.audiosr.clap.open_clip import tokenize
    ...
    if _ROBERTA_TOKENIZER is None:
        ...
        if not roberta_path.is_dir():
            raise FileNotFoundError(
                "AudioSR CLAP text tokenizer requires local roberta-base files. "
                f"Expected directory: {roberta_path}"
            )
        _ROBERTA_TOKENIZER = RobertaTokenizer.from_pretrained(
            str(roberta_path),
...
def tokenize(text, *args, **kwargs):
    return get_roberta_tokenizer()(text, *args, **kwargs)
