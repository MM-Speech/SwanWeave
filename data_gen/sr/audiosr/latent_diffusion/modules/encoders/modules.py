...
from data_gen.sr.audiosr.clap.open_clip import create_model
import torchaudio
from transformers import RobertaTokenizer, AutoTokenizer, T5EncoderModel
import torch.nn.functional as F
from data_gen.sr.audiosr.latent_diffusion.modules.audiomae.AudioMAE import Vanilla_AudioMAE
from data_gen.sr.audiosr.latent_diffusion.modules.phoneme_encoder.encoder import TextEncoder
from data_gen.sr.audiosr.latent_diffusion.util import instantiate_from_config
from data_gen.sr.audiosr.audiosr_utils import get_audiosr_aux_model_path
...
def get_audio_features(audio_data, mel, max_len, data_truncating, data_filling, audio_cfg):
    ...
    sample["longer"] = torch.tensor([True])
...
class CLAPAudioEmbeddingClassifierFreev2(nn.Module):
    ...
    def tokenizer(self, text):
        if self.tokenize is None:
            roberta_path = get_audiosr_aux_model_path("roberta-base")
            if not roberta_path.is_dir():
                raise FileNotFoundError(
                    "AudioSR text tokenizer requires local roberta-base files. "
                    f"Expected directory: {roberta_path}"
                )
