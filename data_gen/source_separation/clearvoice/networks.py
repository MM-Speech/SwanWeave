...
from data_gen.source_separation.common import clearvoice_legacy_checkpoint_dirs, normalize_device
...
class SpeechModel:
    ...
    def __init__(self, args):
        ...
        """
        requested_device = getattr(args, 'device', 'cuda' if getattr(args, 'use_cuda', 1) else 'cpu')
        self.device = normalize_device(requested_device)
        args.device = str(self.device)
        args.use_cuda = int(self.device.type != 'cpu')
        args.num_gpu = 1 if self.device.type == 'cuda' else 0
    ...
    def download_model(self, model_name):
        if not getattr(self.args, 'allow_download', False):
            return False
        ...
        try:
            snapshot_download(repo_id=f'alibabasglab/{model_name}', local_dir=str(checkpoint_dir))
            return True
        except:
            return False
    ...
    def load_model(self):
        ...
        """
        checkpoint_dir = Path(self.args.checkpoint_dir)
        best_name = checkpoint_dir / 'last_best_checkpoint'
        legacy_hints = "\n".join(f"- {path}" for path in clearvoice_legacy_checkpoint_dirs(self.name))

        # Check if the last best checkpoint exists
        if not best_name.is_file() and not self.download_model(self.name):
            raise FileNotFoundError(
                f"clearvoice model `{self.name}` 缺少 checkpoint 索引文件。\n"
                f"resolved checkpoint_dir: {checkpoint_dir}\n"
                f"expected file: {best_name.name}\n"
                f"auto download: disabled by default\n"
                f"legacy fallback dirs:\n{legacy_hints}"
            )

        if isinstance(self.model, nn.ModuleList):
            with best_name.open('r') as f:
    ...
    def _load_model(self, model, checkpoint_path, model_key=None):
        ...
        if not checkpoint_path.is_file():
            raise FileNotFoundError(
                f"clearvoice model `{self.name}` 缺少 checkpoint 文件。\n"
                f"resolved checkpoint_dir: {self.args.checkpoint_dir}\n"
                f"expected file: {checkpoint_path.name}"
            )
        ...
        for key in self.result:
            if use_key:
                ...
                if isinstance(self.result[key], list):  # For multi-speaker outputs
                    ...
                    self.write_audio(output_file, key)
            else:
