...
from data_gen.source_separation.common import normalize_device, resolve_clearvoice_checkpoint_dir, resolve_tool_root
...
class network_wrapper(nn.Module):
    ...
    def __init__(self, device='cuda', model_root=None, allow_legacy_fallback=True, allow_download=False):
        """
    ...
    def finalize_args(self):
        resolved_device = normalize_device(self.device or ('cuda' if self.args.use_cuda else 'cpu'))
        self.args.device = str(resolved_device)
        self.args.use_cuda = int(resolved_device.type != 'cpu')
        self.args.num_gpu = 1 if resolved_device.type == 'cuda' else 0
        self.args.model_root = str(resolve_tool_root('clearvoice', self.model_root))
        self.args.allow_legacy_fallback = self.allow_legacy_fallback
        self.args.allow_download = self.allow_download
        self.args.checkpoint_dir = str(
            resolve_clearvoice_checkpoint_dir(
                self.model_name,
                self.model_root,
