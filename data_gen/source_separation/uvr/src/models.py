...
models_json_path = current_path / "models_dir" / "models.json"
...
class BaseModel:
    def __init__(self, name:str, architecture:str, other_metadata:dict, device=None, logger=None):
        ...
        self.model_path = resolve_uvr_weight_dir(architecture, name)
...
class Demucs(BaseModel):
    ...
    def __init__(self, other_metadata:dict, name:str="htdemucs", device=None, logger=None):
        super().__init__(name, architecture="demucs", other_metadata=other_metadata, device=device, logger=logger)
        self.model_path = str(resolve_uvr_weight_dir("demucs", name))
...
class VrNetwork(BaseModel):
    def __init__(self, other_metadata:dict, name:str="1_HP-UVR", device=None, logger=None):
        ...
        Args:
            ...
            logger (_type_, optional): logger. Defaults to None.
        """
        super().__init__(name, architecture="vr_network", other_metadata=other_metadata, device=device, logger=logger)
        model_path = str(resolve_uvr_weight_dir("vr_network", name))
...
class MDX(BaseModel):
    models_data = mdx_api.load_mdx_models_data(
        model_path=str(current_path / "models_dir" / "mdx" / "modelparams" / "model_data.json")
    )

    def __init__(self, other_metadata: dict, name="UVR-MDX-NET-Inst_1", device=None, logger=None):
        super().__init__(name, architecture="mdx", other_metadata=other_metadata, device=device, logger=logger)
        self.sample_rate = 44100
        model_path = str(resolve_uvr_weight_dir("mdx", name))
...
class MDXC(BaseModel):
    models_data = mdxc_api.load_mdxc_models_data(
        model_path=str(current_path / "models_dir" / "mdxc" / "modelparams" / "model_data.json")
    )
    ...
    def __init__(self, name: str, other_metadata: dict, device=None, logger=None):
        ...
        self.sample_rate = 44100
        model_path = str(resolve_uvr_weight_dir("mdxc", name))
        file_name = os.listdir(model_path)[0]
        model_path = os.path.join(model_path, file_name)
        model_hash = mdxc_api.get_model_hash_from_path(model_path=model_path)

        model_prams_dir = os.path.join(current_path, "models_dir", "mdxc", "modelparams") 
    ...
    def predict_path(self, audio: str, **kwargs) -> dict:
        audio, sampling_rate = read(audio, target_sampling_rate=self.sample_rate)
        return self.predict(audio, sampling_rate)
