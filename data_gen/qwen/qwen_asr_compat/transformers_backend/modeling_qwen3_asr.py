...
from transformers.utils.generic import TransformersKwargs, check_model_inputs as _transformers_check_model_inputs
...
def check_model_inputs(*args, **kwargs):
    """Compatibility wrapper for transformers 4.56.x and 4.57.x.

    Upstream Qwen3-ASR uses ``@check_model_inputs()``, while transformers 4.56.x
    exposes ``check_model_inputs`` as a direct decorator that requires ``func``.
    """
    try:
        return _transformers_check_model_inputs(*args, **kwargs)
    except TypeError as exc:
        if args or kwargs or "missing 1 required positional argument: 'func'" not in str(exc):
            raise
...
class Qwen3ASRThinkerForConditionalGeneration(Qwen3ASRPreTrainedModelForConditionalGeneration, GenerationMixin):
    ...
    def get_audio_features(
        ...
        feature_lens = torch.clamp(feature_lens, min=0, max=input_features.shape[-1])
        ...
        for input_feature, feature_len in zip(input_features, feature_lens):
            feature_len_int = int(feature_len.detach().cpu().item())
    ...
    def forward(
        ...
        **kwargs,
        ...
        if input_features is not None:
            ...
            if _env_flag("SCRIPTSPEECH_QWEN_ALIGNER_SHAPE_ASSERT"):
                audio_mask_token_count = int(audio_mask[..., 0].sum().detach().cpu().item())
                audio_mask_element_count = int(audio_mask.sum().detach().cpu().item())
                audio_feature_token_count = int(audio_features.shape[0]) if audio_features.ndim >= 2 else 0
                audio_feature_element_count = int(audio_features.numel())
                if (
                    audio_mask_token_count != audio_feature_token_count
                    or audio_mask_element_count != audio_feature_element_count
                ):
                    raise ValueError(
                        "Qwen3ASR masked_scatter audio shape mismatch: "
                        f"audio_mask_token_count={audio_mask_token_count}, "
                        f"audio_feature_token_count={audio_feature_token_count}, "
                        f"audio_mask_element_count={audio_mask_element_count}, "
                        f"audio_feature_element_count={audio_feature_element_count}, "
                        f"input_ids_shape={None if input_ids is None else tuple(input_ids.shape)}, "
                        f"inputs_embeds_shape={tuple(inputs_embeds.shape)}, "
                        f"audio_features_shape={tuple(audio_features.shape)}, "
                        f"input_features_shape={None if input_features is None else tuple(input_features.shape)}, "
                        f"feature_attention_mask_shape={None if feature_attention_mask is None else tuple(feature_attention_mask.shape)}, "
                        f"feature_attention_lengths={None if feature_attention_mask is None else torch.sum(feature_attention_mask, dim=1).detach().cpu().tolist()}"
                    )
