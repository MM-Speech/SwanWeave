...
class UnetMid(nn.Module):
    ...
    def forward(self, x, mid_kwargs=None, **kwargs):
        # x [B, T, C]
        if self.is_BTC:
            x = self.pre(x.transpose(1, 2)).transpose(1, 2)
        else:
            x = self.pre(x)
        if mid_kwargs is not None:
            x = self.net(x, **mid_kwargs)
        else:
            x = self.net(x)
        if self.is_BTC:
...
class UnetUp(nn.Module):
    def __init__(self, hidden_size, n_layers, kernel_size, up_rates, channel_multiples=None, dropout=0.0,
        ...
        self.hidden_size = hidden_size
        self.is_BTC = is_BTC
        self.use_skip_layer = use_skip_layer
        ...
        self.in_channels_lst = (np.cumprod([1] + list(channel_multiples)) * hidden_size).astype(int) if not constant_channels \
    ...
    def forward(self, x, skips, **kwargs):
        ...
        for i in range(self.n_layers):
            ...
            if self.use_skip_layer:
                skip_x = (self.skip_layers[i](skip_x) + skip_x) * self.skip_scale
            else:
                skip_x = skip_x * self.skip_scale
