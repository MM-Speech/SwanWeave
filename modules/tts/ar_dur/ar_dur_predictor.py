...
from modules.tts.ar_dur.commons.layers import Embedding, LayerNorm
from modules.tts.ar_dur.commons.nar_tts_modules import PosEmb
from modules.tts.ar_dur.commons.rot_transformer import RotTransformerDecoderLayer
from modules.tts.ar_dur.commons.transformer import SinusoidalPositionalEmbedding, TransformerDecoderLayer
from modules.tts.ar_dur.commons.rel_transformer import RelTransformerEncoder
from modules.tts.ar_dur.commons.align_ops import expand_states, clip_seq_to_multiple
from modules.tts.ar_dur.ps2vq import PsdCodePredictor
import numpy as np
...
class CodePredictor(PsdCodePredictor):
    def __init__(self, hparams, hidden_size, dec_hidden_size, lm_num_layers, dict_size, code_size, precision=torch.float16):
        super(PsdCodePredictor, self).__init__()
    ...
    def forward(self, txt_tokens, ling_feas, char_tokens, ph2char, bert_embed,
                ...
                incremental_state=None, x_ling=None, attn_mask=None):
        ...
        x_ling = x_ling[:, :prev_code.shape[1]]
        ...
        for layer in self.layers:
            if incremental_state is None:
                ...
                self_attn_mask = self_attn_mask + (1 - attn_mask.type(self.precision)) * -1e8
    ...
    def forward_ling_encoder(
        ...
        ph_nonpadding = (ph_tokens > 0).type(self.precision)[:, :, None]  # [B, T_phone, 1]
        ...
        if not hparams.get('lm_use_enc'):
            ...
            x_ph = self.encoder(ph_tokens, other_embeds=ph_enc_oembed.type(self.precision))
        # enc_char
        if char_tokens is not None and ph2char is not None:
            char_nonpadding = (char_tokens > 0).type(self.precision)[:, :, None]
    ...
    def infer(self, txt_tokens, ling_feas, char_tokens, ph2char, bert_embed,
              ...
              incremental_state=None, ctx_vqcodes=None, **kwargs):
        if incremental_state is None:
            incremental_state = {}
        x_ling = self.forward_ling_encoder(
        ...
        if incremental_state != {}:
            ...
            ctx_vqcodes = None
        for step in tqdm(range(vq_decoded.shape[1] - 1), desc='Code LM inference...'):
            vq_pred = self(txt_tokens, None, None, None, None,
                           vq_decoded[:, :step + 1], None, None, None,
                           incremental_state=incremental_state, x_ling=x_ling, **kwargs)
...
class ARDurPredictor(CodePredictor):
    def __init__(self, hparams, hidden_size, dec_hidden_size, lm_num_layers, dict_size, code_size, use_rot_embed=True,
                 op_version=1, **kwargs):
        super().__init__(hparams, hidden_size, dec_hidden_size, lm_num_layers, dict_size, code_size, **kwargs)
    ...
    def infer(self, txt_tokens, ling_feas, char_tokens, ph2char, bert_embed,
              ...
              first_step_min=0, return_probs=False, first_decoder_inp=None, dur_disturb=0.0, use_tqdm=False, **kwargs):
        if incremental_state is None:
            incremental_state = {}
        x_ling = self.forward_ling_encoder(
            txt_tokens, ling_feas, char_tokens, ph2char, bert_embed,
        ...
        probs = []
        if use_tqdm:
            it = tqdm(range(decoded.shape[1] - 1), desc='AR Duration Predictor inference...')
        else:
            it = range(decoded.shape[1] - 1)
        for step in it:
            vq_pred = self(txt_tokens, None, None, None, None,
    ...
    def infer_with_aux(self, txt_tokens, ling_feas, char_tokens, ph2char, bert_embed,
                       spk_id=None, spk_embed=None, mels_timbre=None,
                       incremental_state=None, ctx_vqcodes=None, spk_pos_ids_flat=None,
                       aux_ctx_dur_tokens=None, aux_incremental_state_dur=None, aux_spk_pos_ids_flat=None,
                       aux_weight=None,
                       return_state=False, max_decode_steps=1000000, **kwargs):
        assert aux_incremental_state_dur is not None
        if incremental_state is None:
            incremental_state = {}
        x_ling = self.forward_ling_encoder(
            txt_tokens, ling_feas, char_tokens, ph2char, bert_embed,
        ...
        if incremental_state != {}:
            assert ctx_vqcodes is not None
            vq_decoded[:, :ctx_vqcodes.shape[1]] = ctx_vqcodes
        for step in tqdm(range(min(vq_decoded.shape[1] - 1, max_decode_steps)),
                         desc='AR Duration Predictor inference...'):
            vq_pred = self(txt_tokens, None, None, None, None,
                           vq_decoded[:, :step + 1], None, None, None,
                           incremental_state=incremental_state, x_ling=x_ling,
                           spk_pos_ids_flat=spk_pos_ids_flat, **kwargs)
            aux_vq_pred = self(txt_tokens, None, None, None, None,
                               vq_decoded[:, :step + 1], None, None, None,
                               incremental_state=aux_incremental_state_dur, x_ling=x_ling,
                               spk_pos_ids_flat=aux_spk_pos_ids_flat, **kwargs)
            if ctx_vqcodes is None or step >= ctx_vqcodes.shape[1]:
                print('Dur aux: ', aux_weight)
                vq_pred = torch.round(vq_pred[:, -1, 0] * (1 - aux_weight) + aux_vq_pred[:, -1, 0] * aux_weight).long()
                vq_decoded[:, step + 1] = vq_pred
            else:
                vq_decoded[:, step + 1] = ctx_vqcodes[:, step]
        vq_decoded = vq_decoded[:, 1:]
