...
if importlib.util.find_spec('x_transformers'):
...
class ConvPositionEmbedding(nn.Module):
    ...
    def forward(self, x: float["b n d"], mask: Optional[Bool[Tensor, "b n"]] = None):  # noqa: F722
        if mask is not None:
            mask = mask[..., None]
    ...
    def __call__(
        ...
        if self.use_flash_attn:
            ...
            if mask is not None:
                ...
                x_unpad, indices, cu_seqlens, max_s = unpad_input(x, mask)
                x_unpad = rearrange(x_unpad, 'nnz (three h d) -> nnz three h d', three=3, h=nheads)
                output_unpad = flash_attn_varlen_func(
                    q=x_unpad[:, 0].bfloat16(),
                    k=x_unpad[:, 1].bfloat16(),
                    v=x_unpad[:, 2].bfloat16(),
                    cu_seqlens_q=cu_seqlens,
                    cu_seqlens_k=cu_seqlens,
                    max_seqlen_q=max_s,
                    max_seqlen_k=max_s,
                )
                if isinstance(output_unpad, (tuple, list)):
                    output_unpad = output_unpad[0]

                attn_output = rearrange(pad_input(rearrange(output_unpad, 'nnz h d -> nnz (h d)'),
            ...
            attn_output = attn_output.type_as(query)  # [B, T, heads, C]
