...
class TotalDurationPredictor(nn.Module):
    ...
    def forward(self, inputs, do_checkpoint=False):
        ...
        x = self.decode(txt_tokens, txt_mask, audio_feat, audio_mask, do_checkpoint)
    ...
    def decode(self, txt_tokens, txt_mask, audio_feat, audio_mask, do_checkpoint=False):
        dtype = torch.bfloat16

        txt_embeds = self.text_embedder(txt_tokens).to(dtype)
        txt_embeds = self.text_encoder(txt_embeds, txt_mask, do_checkpoint=do_checkpoint)
...
class Diffusion(nn.Module):
    ...
    def reset_ling_modules(self):
        ...
        self.ling_encoder.apply(self.ling_encoder._init_weights)
