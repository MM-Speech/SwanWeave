...
from modules.commons.hf.transformer import RMSNorm
...
'''
去除text_encoder, 开启caption encoder
'''
...
class Diffusion(nn.Module):
    def __init__(self, hp: ModelArgs):
        ...
        if hp.moe_use_ec:
            ...
            config_dict.update(dict(
                ...
                mlp_only_layers=list(hp.mlp_only_layers),
            ...
            from modules.tts.spat_edit.spat_dit_moe import TransformerMoeDiTModel
            config_dict.update(dict(
                use_moe_ffn=hp.use_moe_ffn,
                decoder_sparse_step=hp.decoder_sparse_step,
                mlp_only_layers=list(hp.mlp_only_layers),
                ...
                moe_p=hp.moe_p,
                moe_num_routed=hp.moe_num_routed,
                moe_num_task_experts=hp.moe_num_task_experts,
                moe_task_p=hp.moe_task_p,
                ...
                moe_null_loss_coef=hp.moe_null_loss_coef,
                moe_use_bias_balance=hp.moe_use_bias_balance,
                moe_bias_update_rate=hp.moe_bias_update_rate,
                moe_bias_momentum=hp.moe_bias_momentum,
                moe_bias_clamp=hp.moe_bias_clamp,
                output_router_logits=hp.output_router_logits,
            ))
        ...
        if not hp.train_base:
            self.edit_lat_proj = nn.Linear(self.hp.in_channels * 2, self.hp.encoder_dim)
    ...
    def _init_weights(self) -> None:
        ...
        self.init_edit_lat_proj_from_lat_proj()

    @torch.no_grad()
    def init_edit_lat_proj_from_lat_proj(self) -> bool:
        if not hasattr(self, "edit_lat_proj"):
            return False

        in_channels = int(self.hp.in_channels)
        if self.edit_lat_proj.weight.shape[1] != in_channels * 2:
            raise ValueError(
                f"edit_lat_proj input dim mismatch: got {self.edit_lat_proj.weight.shape[1]}, "
                f"expected {in_channels * 2}"
            )

        self.edit_lat_proj.weight.zero_()
        self.edit_lat_proj.weight[:, :in_channels].copy_(self.lat_proj.weight)
        if self.edit_lat_proj.bias is not None:
            self.edit_lat_proj.bias.copy_(self.lat_proj.bias)
        return True
    ...
    def _get_safe_bf16_cast_policy(self) -> Dict[str, Any]:
        ...
        if bool(getattr(self.hp, "use_moe_ffn", False)):
            ...
            keep_fp32_module_suffixes.add("mlp.t_proj")
            keep_fp32_module_suffixes.add("mlp.t_budget_proj")
            keep_fp32_module_suffixes.add("task_expert_router")
    ...
    @torch.no_grad()
    def update_moe_bias_balance(self, distributed: bool = True) -> Dict[str, Any]:
        summaries = []
        for module in self.modules():
            if module is self:
                continue
            update_fn = getattr(module, "update_moe_bias_balance", None)
            if not callable(update_fn):
                continue
            summary = update_fn(distributed=distributed)
            if isinstance(summary, dict) and summary.get("updated", False):
                summaries.append(summary)
        return {"num_layers": len(summaries), "layers": summaries}
    ...
    def _encode_text(self, inputs: Dict[str, Any], x_mask: torch.Tensor) -> torch.Tensor:
        ...
        return x_mask.new_zeros((*x_mask.shape, self.hp.encoder_dim), dtype=self.lat_proj.weight.dtype)
    ...
    def _encode_cond(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        bsz = int(tgt_len.shape[0])
        device = tgt_len.device
        dtype = self.lat_proj.weight.dtype
        ctx_mask = torch.zeros((bsz, int(attn_mask.shape[1]), 1), device=device, dtype=dtype)
        ctx = torch.zeros((bsz, int(attn_mask.shape[1]), self.hp.in_channels), device=device, dtype=dtype)
        src_lat = None
        if not self.hp.train_base:
            src_lat = inputs.get("src_lat", None)
            if src_lat is None:
                raise KeyError("inputs['src_lat'] is required when hp.train_base=False")
            if src_lat.shape[0] != bsz:
                raise ValueError(f"src_lat batch {src_lat.shape[0]} must match tgt_len batch {bsz}")
            if src_lat.shape[1] != int(attn_mask.shape[1]):
                raise ValueError(
                    f"src_lat length {src_lat.shape[1]} must match target length {int(attn_mask.shape[1])}"
                )
            src_lat = src_lat.to(device=device, dtype=dtype)
    ...
    def _core_dit(
        ...
        *,
        ...
        src_lat: Optional[torch.Tensor] = None,
        ...
        if ctx_mask is None:
            ctx_mask = x.new_zeros((x.shape[0], x.shape[1], 1))
        if ctx is None:
            ctx = x.new_zeros(x.shape)

        if hasattr(self, "edit_lat_proj"):
            if src_lat is None:
                raise ValueError("Edit model requires src_lat for latent concatenation")
            if src_lat.shape != x.shape:
                raise ValueError(f"src_lat shape {tuple(src_lat.shape)} must match x shape {tuple(x.shape)}")
            src_lat = src_lat.to(device=x.device, dtype=x.dtype)
            x = self.edit_lat_proj(torch.cat([x, src_lat], dim=-1))
        else:
            x = x * (1 - ctx_mask)
    ...
    def _apply_cfg(
        ...
        *,
        seq_cfg_w: Optional[Union[Tuple[Optional[float], Optional[float]], float]],
        timestep_annealing_w: Tuple[float, float, float],
    ) -> torch.Tensor:
        if pred_cfg.shape[0] % 3 == 0 and isinstance(seq_cfg_w, (tuple, list)) and len(seq_cfg_w) == 2:
            seq_cfg_w = self._normalize_dual_cfg_w(seq_cfg_w)
            cfg_mode = "dual"
            bsz = pred_cfg.shape[0] // 3
            cond_all, cond_src, uncond = pred_cfg.chunk(3, dim=0)
        elif pred_cfg.shape[0] % 2 == 0:
            seq_cfg_w = self._normalize_single_cfg_w(seq_cfg_w)
            cfg_mode = "single"
            bsz = pred_cfg.shape[0] // 2
            cond_all, uncond = pred_cfg.chunk(2, dim=0)
        else:
            return pred_cfg
        ...
        if t.ndim == 0:
            t = t.view(1)
        if t.shape[0] == pred_cfg.shape[0]:
            t = t[:bsz]
        elif t.shape[0] == 1:
            t = t.expand(bsz)
        elif t.shape[0] != bsz:
            raise ValueError(f"timesteps shape {tuple(t.shape)} incompatible with CFG batch {pred_cfg.shape[0]}")
        ...
        if cfg_mode == "dual":
            w_src = gamma_t * float(seq_cfg_w[0])
            w_inst = gamma_t * float(seq_cfg_w[1])
            return uncond + w_src * (cond_src - uncond) + w_inst * (cond_all - cond_src)

        return uncond + gamma_t * float(seq_cfg_w) * (cond_all - uncond)

    @staticmethod
    def _normalize_dual_cfg_w(
        seq_cfg_w: Optional[Union[Tuple[Optional[float], Optional[float]], float]]
    ) -> Tuple[float, float]:
        if seq_cfg_w is None:
            return 3.0, 3.0
        if isinstance(seq_cfg_w, (tuple, list)):
            if len(seq_cfg_w) == 0:
                return 3.0, 3.0
            if len(seq_cfg_w) == 1:
                value = 3.0 if seq_cfg_w[0] is None else float(seq_cfg_w[0])
                return value, value
            source_w = 3.0 if seq_cfg_w[0] is None else float(seq_cfg_w[0])
            caption_w = 3.0 if seq_cfg_w[1] is None else float(seq_cfg_w[1])
            return source_w, caption_w
        value = float(seq_cfg_w)
        return value, value

    @staticmethod
    def _normalize_single_cfg_w(seq_cfg_w: Optional[Union[Tuple[Optional[float], ...], float]]) -> float:
        if seq_cfg_w is None:
            return 3.0
        if isinstance(seq_cfg_w, (tuple, list)):
            for value in reversed(seq_cfg_w):
                if value is not None:
                    return float(value)
            return 3.0
        return float(seq_cfg_w)

    @staticmethod
    def _is_dual_cfg_layout(src_lat: Optional[torch.Tensor], total_bsz: int) -> bool:
        if src_lat is None or total_bsz % 3 != 0 or int(src_lat.shape[0]) != total_bsz:
            return False
        base_bsz = total_bsz // 3
        source_all = src_lat[:base_bsz]
        source_only = src_lat[base_bsz : 2 * base_bsz]
        unconditional = src_lat[2 * base_bsz :]
        return torch.equal(source_all, source_only) and int(torch.count_nonzero(unconditional).item()) == 0
    ...
    def forward(
        ...
        *,
        ...
        seq_cfg_w: Optional[Union[Tuple[Optional[float], Optional[float]], float]] = None,
        ...
        try:
            ...
            if task == "training":
                ...
                src_lat = None
                if not self.hp.train_base:
                    x_gt = inputs.get("tgt_lat", x_gt)
                    src_lat = inputs.get("src_lat", None)
                    if src_lat is None:
                        raise KeyError("inputs['src_lat'] is required when hp.train_base=False")
                    if src_lat.shape != x_gt.shape:
                        raise ValueError(
                            f"inputs['src_lat'] shape {tuple(src_lat.shape)} must match target lat shape {tuple(x_gt.shape)}"
                        )

                x_mask = sequence_mask(inputs["lat_lens"], maxlen=int(x_gt.shape[1]))
                ctx_mask = x_gt.new_zeros((x_gt.shape[0], x_gt.shape[1], 1))
                ctx = torch.zeros_like(x_gt)
            ...
            if task == "denoise_step":
                ...
                if cond_bsz % 3 == 0 and (x_bsz == cond_bsz // 3 or x_bsz == cond_bsz):
                    cfg_repeat = 3
                    base_bsz = cond_bsz // 3
                    seq_cfg_w = self._normalize_dual_cfg_w(seq_cfg_w)
                elif cond_bsz % 2 == 0:
                ...
                if cfg_repeat > 1:
                    if x_bsz == base_bsz:
                        x_model = torch.cat([x] * cfg_repeat, dim=0)
                    elif x_bsz == cond_bsz:
                        x_model = x
                    else:
                ...
                src_lat = cond.get("src_lat", None)
                if src_lat is not None:
                    src_bsz = int(src_lat.shape[0])
                    if cfg_repeat > 1 and src_bsz == base_bsz:
                        src_lat = torch.cat([src_lat] * cfg_repeat, dim=0)
                    elif src_bsz != int(x_model.shape[0]):
                        raise ValueError(
                            f"src_lat batch {src_bsz} is incompatible with model batch {int(x_model.shape[0])}"
                        )
    ...
    def inference(
        ...
        **kwargs,
        ...
        dual_cfg_layout = self._is_dual_cfg_layout(cond.get("src_lat", None), total_bsz)

        dual_cfg_requested = (
            seq_cfg_w is None
            or not isinstance(seq_cfg_w, (tuple, list))
            or (isinstance(seq_cfg_w, (tuple, list)) and len(seq_cfg_w) == 2)
        )
        if total_bsz % 3 == 0 and (dual_cfg_layout or (dual_cfg_requested and total_bsz % 2 != 0)):
            bsz = total_bsz // 3
            seq_cfg_w = self._normalize_dual_cfg_w(seq_cfg_w)
        elif total_bsz % 2 == 0:
            bsz = total_bsz // 2
