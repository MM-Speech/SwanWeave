...
def build_dit_model(hparams):
    ...
    config.use_content_cfg_mask_token = hparams.get('use_content_cfg_mask_token', False)
...
class Diffusion(nn.Module):
    def __init__(self, hp: ModelArgs):
        ...
        self.x_prenet = nn.Linear(hp.in_channels + hp.encoder_dim, hp.encoder_dim)
        ...
        # text encoder
        self.content_pre_net = nn.Embedding(16384, 1024)
        self.content_upsampler = nn.Upsample(scale_factor=2, mode='nearest')
        ...
        if hp.use_content_cfg_mask_token:
            self.content_cfg_mask_token = nn.Parameter(torch.zeros((1, 1, hp.encoder_dim)))
    ...
    def forward(self, inputs, sigmas=None, x_noisy=None):
        ...
        content_embed = self.content_pre_net(semantic_tokens)
        content_embed = self.content_upsampler(content_embed.transpose(1, 2)).transpose(1, 2)
        content_cfg_mask = inputs['content_cfg_mask']
        content_embed = content_embed * (1 - content_cfg_mask) + self.content_cfg_mask_token * content_cfg_mask
        content_embed = torch.cat([torch.zeros((bsz, self.hp.num_patches, content_embed.shape[2]), device=device), content_embed], dim=1)

        cond = local_cond + content_embed

        x = self.x_prenet(torch.cat((x_noisy, cond), dim=-1)) + self.f5_time_embed(t)[:, None, :]
    ...
    def _forward(self, x, cond, timesteps, ctx_mask, seq_cfg_w=[1.0, 1.0]):
        """ When we use torchdiffeq, we need to include the CFG process inside _forward() """
        x = x * (1 - ctx_mask)
        x = self.x_prenet(torch.cat((x, cond), dim=-1)) + self.f5_time_embed(timesteps)[:, None, :]
    ...
    def inference(self, inputs, timesteps=20, seq_cfg_w=[1.0, 1.0], **kwargs):
        ...
        content_embed = self.content_pre_net(semantic_tokens)
        content_embed = self.content_upsampler(content_embed.transpose(1, 2)).transpose(1, 2)
        print(content_embed.shape)
        bsz, tgt_len, _ = content_embed.shape
        device = content_embed.device
        content_cfg_mask = torch.zeros((bsz, 1, 1), device=device)
        content_cfg_mask[2:] = 1
        content_embed = content_embed * (1 - content_cfg_mask) + self.content_cfg_mask_token * content_cfg_mask
        content_embed = torch.cat([torch.zeros((bsz, self.hp.num_patches, content_embed.shape[2]), device=device), content_embed], dim=1)
        ...
        t_schedule = torch.linspace(0, 1, timesteps + 1, device=device, dtype=content_embed.dtype)
        if sway_sampling_coef is not None:
            t_schedule = t_schedule + sway_sampling_coef * (torch.cos(torch.pi / 2 * t_schedule) - 1 + t_schedule)
        
        traj = torchdiffeq.odeint(
            lambda t, x: self._forward(
                torch.cat([x] * bsz), cond=local_cond + content_embed, timesteps=t.unsqueeze(0), ctx_mask=ctx_mask, seq_cfg_w=seq_cfg_w),
