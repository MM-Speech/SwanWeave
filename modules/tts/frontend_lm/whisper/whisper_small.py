...
class LayerNorm(nn.LayerNorm):
    def __init__(self, *args, **kwargs):
        ...
        # prevent layernorm params from being converted to fp32
        self.convert = False

    def forward(self, x: Tensor) -> Tensor:
        if not self.convert:
            self.weight.data = self.weight.data.type(torch.float32)
            self.bias.data = self.bias.data.type(torch.float32)
            self.convert = True
...
class Whisper(nn.Module):
    def __init__(self):
        ...
        #     self.n_text_layer, self.n_text_head, dtype=torch.bool
        # )
        # all_heads[self.n_text_layer // 2 :] = True
    ...
    #         self.n_text_layer, self.n_text_head
    #     )
