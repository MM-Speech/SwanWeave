...
def build_wavvae(hparams, infer=False, attn_implementation='flash_attention_2', verbose=True):
    ...
    config = ModelArgs(
        ...
        encoder_resample_mode=hparams.get('encoder_resample_mode', hparams.get('decoder_resample_mode', 'linear')),
...
class ModelArgs:
    ...
    encoder_resample_mode: str = 'linear'
...
class DecoderBlock(nn.Module):
    def __init__(self, config: ModelArgs, input_dim: int = 16, output_dim: int = 8, stride: int = 1, kernel_size: int = 7):
        super().__init__()
        if config.decoder_use_aa_activation:
            pre_act = AASnake1d(
                input_dim,
                oversample_factor=config.decoder_aa_activation_oversample_factor,
                filt_taps=config.decoder_aa_activation_taps,
                cutoff_scale=config.decoder_aa_activation_cutoff_scale,
                resample_mode=config.decoder_resample_mode,
            )
        else:
            pre_act = Snake1d(input_dim)

        if config.decoder_use_antialias_upsample and stride > 1:
            upsample = AntiAliasedUpsampleConv1d(
                input_dim,
                output_dim,
                stride=stride,
                kernel_size=config.kernel_size,
...
class Decoder(nn.Module):
    def __init__(
        ...
        for i, stride in enumerate(rates):
            ...
            layers += [DecoderBlock(config, input_dim, output_dim, stride, config.kernel_size)]
...
class InferenceWrapper(WavVAE):
    ...
    def decode(
        ...
        # 3) crossfade stitch back per sample
        ...
        for b in range(B):
            ...
            for ci, k in enumerate(idxs):
                w = wav_chunks[k, 0]
                start_S = decode_chunk_start_frames[k] * int(self.hop_length)
                end_S = start_S + w.numel()
                if start_S < 0 or end_S > target_S_full:
                    raise RuntimeError(
                        f"Invalid decode chunk placement for sample {b}, chunk {ci}: "
                        f"start_S={start_S}, end_S={end_S}, target={target_S_full}."
                    )

                if ci == 0:
                    audio[b, 0, start_S:end_S] = w
                    current_end = end_S
                    continue

                if start_S > current_end:
                    raise RuntimeError(
                        f"Decode chunk schedule has an audio gap for sample {b}: "
                        f"current_end={current_end}, next_start={start_S}."
                    )

                overlap_len = current_end - start_S
                if overlap_len > w.numel():
                    raise RuntimeError(
                        f"Invalid decode overlap for sample {b}, chunk {ci}: "
                        f"overlap_len={overlap_len}, chunk_len={w.numel()}, overlap_S={overlap_S}."
                    )

                if overlap_len > 0:
                    fade_in = torch.linspace(0.0, 1.0, overlap_len, device=w.device, dtype=w.dtype)
                    fade_out = 1.0 - fade_in
                    prev = audio[b, 0, start_S:current_end]
                    audio[b, 0, start_S:current_end] = prev * fade_out + w[:overlap_len] * fade_in

                if end_S > current_end:
                    audio[b, 0, current_end:end_S] = w[overlap_len:]
                    current_end = end_S

            if current_end != target_S_full:
                raise RuntimeError(
                    f"Decode stitch audio length mismatch for sample {b}: "
                    f"got {current_end}, expected {target_S_full}."
                )
...
def compute_wavvae_window_size(config, seq_len_samples=None):
    ...
    encoder_resample_mode = str(getattr(config, "encoder_resample_mode", getattr(config, "decoder_resample_mode", "linear")))
    _check_resample_mode(encoder_resample_mode)
    if aa and k0 % 2 == 0:
...
def _check_resample_mode(mode: str) -> str:
    mode = str(mode).lower()
    if mode not in {"linear", "nearest", "nearest-exact"}:
...
def compute_wavvae_decoder_window_size(config):
    ...
    def act(iv):
        return _backprop_aa_snake_interval(iv, cfg["aa_activation_oversample_factor"], cfg["aa_activation_taps"], cfg["resample_mode"]) if cfg["aa_activation"] else iv
    def res(iv, k, d):
        iv = act(iv)
    ...
    for phase in range(up):
        ...
        iv = act(iv)
        for s, k, ds, taps in reversed(list(zip(cfg["decoder_rates"], cfg["decoder_kernel_sizes"], cfg["decoder_dilations"], cfg["aa_upsample_taps"]))):
            for d in reversed(ds):
                iv = res(iv, k, d)
            if cfg["aa_upsample"] and s > 1:
                iv = _backprop_same_conv_interval(iv, kernel_size=cfg["aa_proj_kernel_size"])
                iv = _backprop_same_conv_interval(iv, kernel_size=taps)
                iv = _backprop_interpolate_scale_up_interval(iv, stride=s, mode=cfg["resample_mode"])
            else:
