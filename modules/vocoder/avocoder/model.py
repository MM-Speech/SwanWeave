...
from .modules import PQMF, CoMBD, SubBandDiscriminator, get_padding, init_weights
...
class Generator(torch.nn.Module):
    def __init__(self, h, c_out=1):
        ...
        h['nsf_type'] = h.get('nsf_type', 'none')
        if h['nsf_type'] != 'none':
            ...
            c_out_noise = 1
            for i, u in enumerate(h['upsample_rates'][::-1]):
                c_in_noise = c_out_noise
                c_out_noise = h['upsample_initial_channel'] // (2 ** (self.num_upsamples - i - 1))
                self.noise_convs.append(nn.Conv1d(c_in_noise, c_out_noise, u * 2, u, padding=u // 2))
    ...
    def forward(self, x, f0=None, *args, **kwargs):
        ...
        x = self.conv_post(x)
        x = torch.tanh(x)
        x = x[:, :, :mel.shape[2] * self.h['hop_size']]
        return x, x2, x1
