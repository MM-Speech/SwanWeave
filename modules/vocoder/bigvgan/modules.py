...
class AMPBlock_filter(torch.nn.Module):
    def __init__(self, channels, kernel_size=3, dilation=(1, 3, 5), orig_freq=None):
        ...
        self.alpha1 = nn.ModuleList([nn.Parameter(torch.ones(1, channels, 1)) for i in range(len(self.convs1))])
        self.alpha2 = nn.ModuleList([nn.Parameter(torch.ones(1, channels, 1)) for i in range(len(self.convs2))])
