...
class PFODESolver():
    ...
    def get_timesteps(self, t_start, t_end, num_steps):
        ...
        timesteps = (self.scheduler.num_train_timesteps - 1) + (
                    timepoints - self.t_initial) / self.stepsize  # correspondint to StableDiffusion indexing system, from 999 (t_init) -> 0 (dt)
        ...
        with torch.no_grad():
            ...
            for i in range(num_steps):
                ...
                for ib in range(prev_timestep.shape[0]):
                    alpha_prod_t_prev[ib] = self.scheduler.alphas_cumprod[prev_timestep[ib]] if prev_timestep[
                                                                                                    ib] >= 0 else self.scheduler.final_alpha_cumprod
                ...
                if self.scheduler.config.prediction_type == "epsilon":
                    pred_original_sample = (latents - beta_prod_t[:, None, None, None] ** (
                        0.5) * noise_pred) / alpha_prod_t[:, None, None, None] ** (0.5)
                ...
                #     pred_epsilon = (latents - alpha_prod_t ** (0.5) * pred_original_sample) / beta_prod_t ** (0.5)
                    ...
                    pred_original_sample = (alpha_prod_t[:, None, None, None] ** 0.5) * latents - (
                                beta_prod_t[:, None, None, None] ** 0.5) * noise_pred
                    pred_epsilon = (alpha_prod_t[:, None, None, None] ** 0.5) * noise_pred + (
                                beta_prod_t[:, None, None, None] ** 0.5) * latents
                else:
                    raise ValueError(
...
class PFODESolverSDXL():
    ...
    def solve(self,
        ...
        add_time_ids = torch.cat(
            ...
            [self._get_add_time_ids((resolution, resolution), (0, 0), (resolution, resolution), dtype) for _ in
             range(bsz)]
        ).to(device)
